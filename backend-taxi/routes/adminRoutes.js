...
router.get('/list-taxi', authMiddleware, (req, res) => {
  // Vérifiez si l'utilisateur est un administrateur ou un propriétaire
  if (!['admin', 'proprietaire'].includes(req.user.type)) {
    return res.status(403).json({ message: 'Accès interdit.' });
  }

  // Requête pour récupérer tous les taxis
  const query = `SELECT id, nom, matricule, photo_de_taxi, prix, created_at FROM taxi`;
  db.execute(query, (err, results) => {
    if (err) {
